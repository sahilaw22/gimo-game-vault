...
import { Copy, Eye, EyeOff, Edit3, Trash2, Check, Key, Mail, User, Info, ShieldAlert, Heart } from 'lucide-react';
...
export const CredentialCard: React.FC<CredentialCardProps> = ({
  ...
  return (
    <div className="bg-[#12151c]/90 backdrop-blur-md rounded-xl p-4 border border-zinc-800/80 hover:bg-[#181b24] hover:border-zinc-700/80 transition-all duration-200 shadow-md hover:shadow-xl flex flex-col justify-between group">
      ...
      <div>
          ...
          <div className="flex-1 min-w-0">
            ...
            {item.tags && item.tags.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mt-1.5">
                {item.tags.map((tag) => getPlatformBadge(tag))}
              </div>
            )}
          </div>

          <div className="flex items-center gap-1 bg-zinc-900/60 border border-zinc-800/60 rounded-lg p-1">
            {/* Heart Favorite Button */}
            {onToggleFavorite && (
              <button
                onClick={() => onToggleFavorite(item.game)}
                title={item.isFavorite ? "Remove from Favourites" : "Mark as Favourite"}
                className="p-1 text-zinc-400 hover:text-red-500 rounded-md transition-colors"
              >
                <Heart className={`w-3.5 h-3.5 transition-all ${item.isFavorite ? 'fill-red-500 text-red-500 scale-110' : ''}`} />
              </button>
            )}
        ...
        <div className="space-y-2 mt-3">
          ...
          {/* Account/ID */}
          {item.account && (
            <div className="bg-zinc-900/40 border border-zinc-800/60 rounded-lg p-2 flex items-center justify-between gap-2">
              <div className="flex items-center gap-2 min-w-0">
                <User className="w-3.5 h-3.5 text-zinc-400 flex-shrink-0" />
                <div className="min-w-0">
                  <div className="text-[9px] font-mono text-zinc-500 uppercase tracking-wider">Account / ID</div>
                  <div className="text-xs font-mono text-zinc-200 font-medium truncate">{item.account}</div>
                </div>
              </div>
              <button
                onClick={() => handleCopy(item.account, 'account')}
                className="flex items-center gap-1 text-[10px] font-mono bg-zinc-800/80 hover:bg-zinc-700 text-zinc-300 hover:text-white px-2 py-0.5 rounded border border-zinc-700/50 transition"
              >
          ...
          {item.password && (
            <div className="bg-zinc-900/40 border border-zinc-800/60 rounded-lg p-2 flex items-center justify-between gap-2">
              <div className="flex items-center gap-2 min-w-0">
                ...
                <div className="min-w-0">
                  <div className="text-[9px] font-mono text-zinc-500 uppercase tracking-wider">Password</div>
                  <div className="text-xs font-mono text-zinc-200 font-medium truncate">
              ...
              <div className="flex items-center gap-1 flex-shrink-0">
                <button
                  ...
                  className="p-1 text-zinc-400 hover:text-white bg-zinc-800/80 rounded border border-zinc-700/50 transition"
                >
          ...
          {item.email && (
            <div className="bg-zinc-900/40 border border-zinc-800/60 rounded-lg p-2 flex items-center justify-between gap-2">
              <div className="flex items-center gap-2 min-w-0">
                ...
                <div className="min-w-0">
                  <div className="text-[9px] font-mono text-zinc-500 uppercase tracking-wider">Email</div>
                  <div className="text-xs font-mono text-zinc-200 font-medium truncate">{item.email}</div>
                </div>
              </div>
              <button
          ...
          {item.emailPass && (
            <div className="bg-zinc-900/40 border border-zinc-800/60 rounded-lg p-2 flex items-center justify-between gap-2">
              <div className="flex items-center gap-2 min-w-0">
                ...
                <div className="min-w-0">
                  <div className="text-[9px] font-mono text-zinc-500 uppercase tracking-wider">Email Pass</div>
                  <div className="text-xs font-mono text-zinc-200 font-medium truncate">
              ...
              <div className="flex items-center gap-1 flex-shrink-0">
                ...
                <button
                  ...
                  className="flex items-center gap-1 text-[10px] font-mono bg-zinc-800/80 hover:bg-zinc-700 text-zinc-300 hover:text-white px-2 py-0.5 rounded border border-zinc-700/50 transition"
                >
                  {copiedField === 'emailPass' ? <Check className="w-3 h-3 text-emerald-400" /> : <Copy className="w-3 h-3" />}
          ...
          {/* Extra Info / Notes */}
          {item.extra && (
            <div className="bg-zinc-900/40 border border-zinc-800/60 rounded-xl p-2.5 flex items-center justify-between gap-2 text-xs font-mono">
              <div className="flex items-center gap-2 text-zinc-400 min-w-0">
                <Info className="w-3.5 h-3.5 text-zinc-400 flex-shrink-0" />
                <div className="min-w-0">
                  <div className="text-[9px] text-zinc-500 uppercase tracking-wider">Notes</div>
                  <span className="truncate text-zinc-200 font-medium text-xs block">{item.extra}</span>
                </div>
              </div>
              <button
                onClick={() => handleCopy(item.extra || '', 'notes')}
                className="flex items-center gap-1 text-[10px] font-mono bg-zinc-800/80 hover:bg-zinc-700 text-zinc-300 hover:text-white px-2 py-0.5 rounded border border-zinc-700/50 transition flex-shrink-0"
              >
                {copiedField === 'notes' ? <Check className="w-3 h-3 text-emerald-400" /> : <Copy className="w-3 h-3" />}
                <span>{copiedField === 'notes' ? 'Copied' : 'Copy'}</span>
              </button>
            </div>
          )}
